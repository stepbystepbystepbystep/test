...
## Папки для музыки (MP3)

Рядом с `TopDownRunner.exe` игра автоматически использует:
- `Audio/BGM` — фоновая музыка (`*.mp3`)
- `Audio/SFX` — музыка эффекта ранца/джетпака (`*.mp3`)

Поведение:
- BGM зациклена: если трек закончился, он стартует снова с начала.
- SFX для джетпака включается при подборе ранца и играет по кругу, пока эффект ранца активен.
- Если в папке несколько mp3, берется первый файл по имени (в алфавитном порядке).

## Визуальные ассеты (пиксель-стиль)

Можно подставить свои PNG рядом с `TopDownRunner.exe`:
- `runner.png` — убегающий
- `chaser.png` — догоняющий
- `obstacle.png` — препятствие
- `jetpack.png` — ранец

Если файлов нет, игра рисует встроенные пиксельные фигуры.

## Что улучшено в геймплее

- Убраны "непроходимые" генерации: безопасная линия между рядами меняется плавно, чтобы маршрут оставался достижимым.
- Догоняющий теперь всегда виден на экране и визуально приближается/отдаляется.
- Добавлена глубина окружения: боковые зоны, дорожные разметки, движущиеся элементы фона.
